...
public class TableroSG implements Tablero {
	...
	public TableroSG (Tablero t){
		...
		for(int i=0; i < t.getColumnas(); i++){
			...
			for(int j=0; j < t.getFilas(); j--){
				colum.put(j , new CasillaSG(t.optenerCasilla(i, j).getColor(),i,j,false));
				//t[i][j]= color; j=x; i=y; 
			}
	...
	public TableroSG (int filas, int columnas){
		...
		for(int i=0; i < columnas; i++){
			...
			for(int j=0; j < filas; j++){
				colum.put(j , new CasillaSG(0,j,i,false));
			}
			tableroSG.put(i,colum);
		}
	}
	...
	public int efectuarJugada(Casilla jugada) throws IllegalArgumentException {
		...
		if (this.tableroSG.get(jugada.getColumna()).get(jugada.getFila()).getColor()==0) {
		...
		System.out.println("-----Vecinos:   "+vecinos.size());
		...
		for (int i = 0; i < vecinos.size(); i++) {
			...
			this.modificarCasilla(vecinos.get(i).getFila(), vecinos.get(i).getColumna(), 0);
		}
	...
	protected void eliminarCasilla (HashMap<Integer, Pareja> eliminar) {
		for(int col: eliminar.keySet()) {
			...
			for(int i =0; i< cantEliminar; i++) {
				this.tableroSG.get(col).get(i).setColor(0);
			}
		}
		for(int col: eliminar.keySet()) {
			if(this.tableroSG.get(col).get(filas-1).getColor()==0) {
				eliminarColumna (col);
			}
		}
	}
	...
	@Override
	public CasillaSG optenerCasilla(int x, int y) {
		return tableroSG.get(y).get(x);
	}
